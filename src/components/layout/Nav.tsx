...
const links = [
  ...
  { href: "/log/activity", icon: Flame, label: "Log Activity" },
...
export default function Nav() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex fixed left-0 top-0 h-full w-64 flex-col border-r z-40"
      ...
      {/* Mobile bottom nav */}
      <div className="fixed bottom-0 left-0 right-0 md:hidden border-t z-40 flex"
        style={{ background: "var(--bg-card)", borderColor: "var(--border)" }}>
        {links.map(({ href, icon: Icon, label }) => {
          const active = path === href || (href !== "/" && path.startsWith(href));
          return (
